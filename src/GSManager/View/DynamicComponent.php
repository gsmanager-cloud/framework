...
class DynamicComponent extends Component
{
     ...
     *
     * @var \GSManager\View\Compilers\BladeTagCompiler
     */
     ...
     *
     * @return \GSManager\Contracts\View\View|string
     */
    public function render()
    {
        $template = <<<'EOF'
<?php extract((new \GSManager\Support\Collection($attributes->getAttributes()))->mapWithKeys(function ($value, $key) { return [GSManager\Support\Str::camel(str_replace([':', '.'], ' ', $key)) => $value]; })->all(), EXTR_SKIP); ?>
{{ props }}
...
EOF;
        ...
        return function ($data) use ($template) {
            ...
            return str_replace(
                ...
                [
                    ...
                    $this->compileSlots($data['__gsmanager_slots']),
                    '{{ $slot ?? "" }}',
                ],
                $template
            );
     ...
     *
     * @return \GSManager\View\Compilers\ComponentTagCompiler
     */
