...
use Closure;
use GSManager\Console\Events\ArtisanStarting;
use GSManager\Contracts\Console\Application as ApplicationContract;
...
class Application extends SymfonyApplication implements ApplicationContract
{
     ...
     */
    public function __construct(Container $gsmanager, Dispatcher $events, $version)
    {
        parent::__construct('GSManager Framework', $version);
     ...
     * @return string
     */
    public static function gsmBinary()
    {
        return ProcessUtils::escapeArgument(gsm_binary());
    }
    ...
    {
        return sprintf('%s %s %s', static::phpBinary(), static::gsmBinary(), $string);
    }
    ...
    protected function parseCommand($command, $parameters)
    {
        if (is_subclass_of($command, SymfonyCommand::class)) {
            ...
            $command = $this->gsmanager->make($command)->getName();
        }
    ...
    public function add(SymfonyCommand $command): ?SymfonyCommand
    {
        if ($command instanceof Command) {
            $command->setGSManager($this->gsmanager);
        }
     ...
     *
     * @param  \GSManager\Console\Command|string  $command
     * @return \Symfony\Component\Console\Command\Command|null
     */
    public function resolve($command)
    {
        ...
        return $this->add($this->gsmanager->make($command));
    }
    ...
    {
        $this->setCommandLoader(new ContainerCommandLoader($this->gsmanager, $this->commandMap));
