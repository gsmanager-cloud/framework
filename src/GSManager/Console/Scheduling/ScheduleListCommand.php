...
class ScheduleListCommand extends Command
{
     ...
     *
     * @param  \GSManager\Support\Collection  $events
    ...
    private function listEvent($event, $terminalWidth, $expressionSpacing, $repeatExpressionSpacing, $timezone)
    {
        ...
        $command = preg_replace("#(php gsm [\w\-:]+) (.+)#", '$1 <fg=yellow;options=bold>$2</>', $command);
     ...
     */
    private function sortEvents(\GSManager\Support\Collection $events, DateTimeZone $timezone)
    {
     ...
     *
     * @param  \GSManager\Console\Scheduling\CallbackEvent  $event
     * @return string
     */
    private function getClosureLocation(CallbackEvent $event)
    {
        ...
        if ($callback instanceof Closure) {
            ...
            return sprintf(
                '%s:%s',
                str_replace($this->gsmanager->basePath().DIRECTORY_SEPARATOR, '', $function->getFileName() ?: ''),
