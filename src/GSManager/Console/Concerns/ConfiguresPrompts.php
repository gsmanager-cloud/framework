...
namespace GSManager\Console\Concerns;

use GSManager\Console\PromptValidationException;
...
trait ConfiguresPrompts
{
    ...
    protected function configurePrompts(InputInterface $input)
    {
        ...
        Prompt::interactive(($input->isInteractive() && defined('STDIN') && stream_isatty(STDIN)) || $this->gsmanager->runningUnitTests());

        Prompt::validateUsing(fn (Prompt $prompt) => $this->validatePrompt($prompt->value(), $prompt->validate));

        Prompt::fallbackWhen(windows_os() || $this->gsmanager->runningUnitTests());
    ...
    protected function promptUntilValid($prompt, $required, $validate)
    {
        while (true) {
            ...
            if ($required && ($result === '' || $result === [] || $result === false)) {
                ...
                if ($this->gsmanager->runningUnitTests()) {
    ...
    {
        return $this->gsmanager['validator']->make(
            [$field => $value],
            [$field => $rules],
    ...
    private function multiselectFallback($label, $options, $default = [], $required = false)
    {
        ...
        if ($required === false && ! $this->gsmanager->runningUnitTests()) {
