...
use GSManager\Http\Client\Events\ConnectionFailed;
use GSManager\Http\Client\Events\RequestSending;
use GSManager\Http\Client\Events\ResponseReceived;
...
class PendingRequest
{
     ...
     *
     * @var \GSManager\Http\Client\Factory|null
     */
    protected $factory;
     ...
     *
     * @var \GSManager\Support\Collection|null
     */
     ...
     *
     * @var \GSManager\Http\Client\Request|null
     */
    protected $request;
     ...
     * @return \GSManager\Http\Client\Response
     *
     * @throws \GSManager\Http\Client\ConnectionException
     */
     ...
     *
     * @param  string  $url
     * @param  array|\JsonSerializable|\GSManager\Contracts\Support\Arrayable  $data
     * @return \GSManager\Http\Client\Response
     *
     * @throws \GSManager\Http\Client\ConnectionException
     */
     ...
     *
     * @param  callable  $callback
     * @return array<array-key, \GSManager\Http\Client\Response>
     */
    ...
    protected function sendRequest(string $method, string $url, array $options = [])
    {
        ...
        $gsmanagerData = $this->parseRequestData($method, $url, $options);
    ...
    protected function parseRequestData($method, $url, array $options)
    {
        ...
        $gsmanagerData = $options[$this->bodyFormat] ?? $options['query'] ?? [];

        $urlString = (new Stringable($url));

        if (empty($gsmanagerData) && $method === 'GET' && $urlString->contains('?')) {
            $gsmanagerData = (string) $urlString->after('?');
        }

        if (is_string($gsmanagerData)) {
            parse_str($gsmanagerData, $parsedData);

            $gsmanagerData = is_array($parsedData) ? $parsedData : [];
        }

        if ($gsmanagerData instanceof JsonSerializable) {
            $gsmanagerData = $gsmanagerData->jsonSerialize();
        }

        return is_array($gsmanagerData) ? $gsmanagerData : [];
    }
            ...
            return function ($request, $options) use ($handler) {
                ...
                return $promise->then(function ($response) use ($request, $options) {
                    $this->factory?->recordRequestResponsePair(
                        (new Request($request))->withData($options['gsmanager_data']),
        ...
        return function ($handler) {
            return function ($request, $options) use ($handler) {
                $response = ($this->stubCallbacks ?? new Collection)
                    ->map
                    ->__invoke((new Request($request))->withData($options['gsmanager_data']), $options)
                    ->filter()
                    ->first();
        ...
        return tap($request, function (&$request) use ($options) {
            $this->beforeSendingCallbacks->each(function ($callback) use (&$request, $options) {
                $callbackResult = call_user_func(
                    $callback, (new Request($request))->withData($options['gsmanager_data']), $options, $this
                );
    ...
    {
        return tap(new Response($response), function (Response $gsmanagerResponse) {
            if ($this->truncateExceptionsAt === null) {
                return;
            }

            $this->truncateExceptionsAt === false
                ? $gsmanagerResponse->dontTruncateExceptions()
                : $gsmanagerResponse->truncateExceptionsAt($this->truncateExceptionsAt);
        });
    }
