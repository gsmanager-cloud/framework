...
use Closure;
use GSManager\Contracts\Log\ContextLogProcessor;
...
class LogManager implements LoggerInterface
{
     ...
     *
     * @param  string  $name
     * @param  \GSManager\Log\Logger  $logger
     * @return \GSManager\Log\Logger
     */
    ...
    protected function createEmergencyLogger()
    {
        ...
        $handler = new StreamHandler(
            $config['path'] ?? $this->app->storagePath().'/logs/gsmanager.log',
            $this->level(['level' => 'debug'])
        );

        return new Logger(
            new Monolog('gsmanager', $this->prepareHandlers([$handler])),
            $this->app['events']
        );
    }
