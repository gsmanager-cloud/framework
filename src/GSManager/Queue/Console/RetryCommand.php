...
use GSManager\Queue\Events\JobRetryRequested;
...
class RetryCommand extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($ids as $id) {
            $job = $this->gsmanager['queue.failer']->find($id);

            if (is_null($job)) {
                $this->components->error("Unable to find failed job with ID [{$id}].");
            } else {
                $this->gsmanager['events']->dispatch(new JobRetryRequested($job));

                $this->components->task($id, fn () => $this->retryJob($job));

                $this->gsmanager['queue.failer']->forget($id);
            }
        }
    ...
    {
        $this->gsmanager['queue']->connection($job->connection)->pushRaw(
    ...
    protected function refreshRetryUntil($payload)
    {
        ...
        } elseif ($this->gsmanager->bound(Encrypter::class)) {
            $instance = unserialize($this->gsmanager->make(Encrypter::class)->decrypt($payload['data']['command']));
        }
