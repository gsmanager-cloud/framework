...
use GSManager\Contracts\Queue\Factory;
use GSManager\Queue\Events\QueueBusy;
...
class MonitorCommand extends Command
{
    ...
    protected function parseQueues($queues)
    {
        return (new Collection(explode(',', $queues)))->map(function ($queue) {
            ...
            if (! isset($queue)) {
                $queue = $connection;
                $connection = $this->gsmanager['config']['queue.default'];
            }
     ...
     *
     * @param  \GSManager\Support\Collection  $queues
     * @return void
     */
