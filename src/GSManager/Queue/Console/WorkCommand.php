...
use GSManager\Queue\Events\JobFailed;
use GSManager\Queue\Events\JobProcessed;
use GSManager\Queue\Events\JobProcessing;
use GSManager\Queue\Events\JobReleasedAfterException;
use GSManager\Queue\Worker;
use GSManager\Queue\WorkerOptions;
...
class WorkCommand extends Command
{
     ...
     *
     * @param  \GSManager\Queue\Worker  $worker
    ...
    public function handle()
    {
        ...
        $connection = $this->argument('connection')
            ?: $this->gsmanager['config']['queue.default'];
     ...
     *
     * @return \GSManager\Queue\WorkerOptions
     */
    ...
    protected function listenForEvents()
    {
        ...
        $this->gsmanager['events']->listen(JobProcessing::class, function ($event) {
            $this->writeOutput($event->job, 'starting');
        });

        $this->gsmanager['events']->listen(JobProcessed::class, function ($event) {
            $this->writeOutput($event->job, 'success');
        });

        $this->gsmanager['events']->listen(JobReleasedAfterException::class, function ($event) {
            $this->writeOutput($event->job, 'released_after_exception');
        });

        $this->gsmanager['events']->listen(JobFailed::class, function ($event) {
    ...
    {
        $queueTimezone = $this->gsmanager['config']->get('queue.output_timezone');

        if ($queueTimezone &&
            $queueTimezone !== $this->gsmanager['config']->get('app.timezone')) {
     ...
     *
     * @param  \GSManager\Queue\Events\JobFailed  $event
     * @return void
     */
    protected function logFailedJob(JobFailed $event)
    {
        $this->gsmanager['queue.failer']->log(
    ...
    {
        return $this->option('queue') ?: $this->gsmanager['config']->get(
    ...
    {
        return $this->option('force') ? false : $this->gsmanager->isDownForMaintenance();
    }
