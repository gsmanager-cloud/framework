...
use GSManager\Contracts\Routing\UrlGenerator as UrlGeneratorContract;
use GSManager\Contracts\View\Factory as ViewFactoryContract;
        ...
        $this->app->bind(ServerRequestInterface::class, function ($app) {
            if (class_exists(PsrHttpFactory::class)) {
                return with((new PsrHttpFactory)
                    ->createRequest($gsmanagerRequest = $app->make('request')), function (ServerRequestInterface $request) use ($gsmanagerRequest) {
                        if ($gsmanagerRequest->getContentTypeFormat() !== 'json' && $gsmanagerRequest->request->count() === 0) {
                            return $request;
                        }

                        return $request->withParsedBody(
                            array_merge($request->getParsedBody() ?? [], $gsmanagerRequest->getPayload()->all())
                        );
                    });
            }
