...
use GSManager\Cache\Events\CacheFlushFailed;
use GSManager\Cache\Events\CacheFlushing;
use GSManager\Cache\Events\CacheHit;
use GSManager\Cache\Events\CacheMissed;
use GSManager\Cache\Events\ForgettingKey;
use GSManager\Cache\Events\KeyForgetFailed;
use GSManager\Cache\Events\KeyForgotten;
use GSManager\Cache\Events\KeyWriteFailed;
use GSManager\Cache\Events\KeyWritten;
use GSManager\Cache\Events\RetrievingKey;
use GSManager\Cache\Events\RetrievingManyKeys;
use GSManager\Cache\Events\WritingKey;
use GSManager\Cache\Events\WritingManyKeys;
...
/**
 * @mixin \GSManager\Contracts\Cache\Store
 */
class Repository implements ArrayAccess, CacheContract
{
     ...
     *
     * @var \GSManager\Contracts\Cache\Store
     */
    protected $store;
    ...
    public function flexible($key, $ttl, $callback, $lock = null, $alwaysDefer = false)
    {
        [
            $key => $value,
            "gsmanager:cache:flexible:created:{$key}" => $created,
        ] = $this->many([$key, "gsmanager:cache:flexible:created:{$key}"]);

        if (in_array(null, [$value, $created], true)) {
            return tap(value($callback), fn ($value) => $this->putMany([
                $key => $value,
                "gsmanager:cache:flexible:created:{$key}" => Carbon::now()->getTimestamp(),
            ], $ttl[1]));
        }
        ...
        $refresh = function () use ($key, $ttl, $callback, $lock, $created) {
            $this->store->lock(
                "gsmanager:cache:flexible:lock:{$key}",
                $lock['seconds'] ?? 0,
                $lock['owner'] ?? null,
            )->get(function () use ($key, $callback, $created, $ttl) {
                if ($created !== $this->get("gsmanager:cache:flexible:created:{$key}")) {
                    return;
                }
        ...
        defer($refresh, "gsmanager:cache:flexible:{$key}", $alwaysDefer);
     ...
     * @return \GSManager\Cache\TaggedCache
     *
     ...
     *
     * @return \GSManager\Contracts\Cache\Store
     */
