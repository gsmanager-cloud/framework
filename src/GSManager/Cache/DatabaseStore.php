...
use GSManager\Database\PostgresConnection;
use GSManager\Database\QueryException;
use GSManager\Database\SQLiteConnection;
...
class DatabaseStore implements LockProvider, Store
{
    ...
    protected function forgetMany(array $keys)
    {
        $this->table()->whereIn('key', (new Collection($keys))->flatMap(fn ($key) => [
            $this->prefix.$key,
            "{$this->prefix}gsmanager:cache:flexible:created:{$key}",
        ])->all())->delete();
    ...
    protected function forgetManyIfExpired(array $keys, bool $prefixed = false)
    {
        $this->table()
            ->whereIn('key', (new Collection($keys))->flatMap(fn ($key) => $prefixed ? [
                $key,
                $this->prefix.'gsmanager:cache:flexible:created:'.Str::chopStart($key, $this->prefix),
            ] : [
                "{$this->prefix}{$key}",
                "{$this->prefix}gsmanager:cache:flexible:created:{$key}",
            ])->all())
