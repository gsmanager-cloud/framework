...
use GSManager\Contracts\Filesystem\LockTimeoutException;
use GSManager\Filesystem\Filesystem;
use GSManager\Filesystem\LockableFile;
...
class FileStore implements Store, LockProvider
{
    ...
    /**
     * The GSManager Filesystem instance.
     *
    ...
    public function forget($key)
    {
        if ($this->files->exists($file = $this->path($key))) {
            return tap($this->files->delete($file), function ($forgotten) use ($key) {
                if ($forgotten && $this->files->exists($file = $this->path("gsmanager:cache:flexible:created:{$key}"))) {
