...
use GSManager\Routing\ViewController;
...
class RouteListCommand extends Command
{
    ...
    protected function isFrameworkController(Route $route)
    {
        return in_array($route->getControllerClass(), [
            '\GSManager\Routing\RedirectController',
            '\GSManager\Routing\ViewController',
        ], true);
    }
     ...
     *
     * @param  \GSManager\Support\Collection  $routes
     * @return string
     */
    ...
    protected function formatActionForCli($route)
    {
        ...
        $rootControllerNamespace = $this->gsmanager[UrlGenerator::class]->getRootControllerNamespace()
            ?? ($this->gsmanager->getNamespace().'Http\\Controllers');
