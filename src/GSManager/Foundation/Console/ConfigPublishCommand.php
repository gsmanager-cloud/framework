...
class ConfigPublishCommand extends Command
{
    ...
    public function handle()
    {
        ...
        if (is_null($this->argument('name')) && $this->option('all')) {
            foreach ($config as $key => $file) {
                $this->publish($key, $file, $this->gsmanager->configPath().'/'.$key.'.php');
            }
        ...
        $this->publish($name, $config[$name], $this->gsmanager->configPath().'/'.$name.'.php');
    }
    ...
    protected function getBaseConfigurationFiles()
    {
        ...
        $shouldMergeConfiguration = $this->gsmanager->shouldMergeFrameworkConfiguration();
