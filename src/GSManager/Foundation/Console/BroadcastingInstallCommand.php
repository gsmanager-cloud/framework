...
use function GSManager\Prompts\text;
...
class BroadcastingInstallCommand extends Command
{
    ...
    protected $signature = 'install:broadcasting
                    ...
                    {--without-reverb : Do not prompt to install GSManager Reverb}
                    {--reverb : Install GSManager Reverb as the default broadcaster}
    ...
    protected $frameworkPackages = [
        'react' => '@gsmanager/echo-react',
        'vue' => '@gsmanager/echo-vue',
    ];
    ...
    public function handle()
    {
        ...
        if (! file_exists($broadcastingRoutesPath = $this->gsmanager->basePath('routes/channels.php')) || $this->option('force')) {
        ...
        Env::writeVariable('BROADCAST_CONNECTION', $this->driver, $this->gsmanager->basePath('.env'), true);
        ...
        if ($this->isUsingSupportedFramework()) {
            ...
            if (! file_exists($echoScriptPath = $this->gsmanager->resourcePath('js/echo.js'))) {
                if (! is_dir($directory = $this->gsmanager->resourcePath('js'))) {
            ...
            if (file_exists($bootstrapScriptPath = $this->gsmanager->resourcePath('js/bootstrap.js'))) {
            ...
            } elseif (file_exists($appScriptPath = $this->gsmanager->resourcePath('js/app.js'))) {
    ...
    {
        $envPath = $this->gsmanager->basePath('.env');
    ...
    protected function injectFrameworkSpecificConfiguration()
    {
        if ($this->appUsesVue()) {
            ...
            $filePaths = [
                $this->gsmanager->resourcePath('js/app.ts'),
                $this->gsmanager->resourcePath('js/app.js'),
            ];
        } else {
            $importPath = $this->frameworkPackages['react'];

            $filePaths = [
                $this->gsmanager->resourcePath('js/app.tsx'),
                $this->gsmanager->resourcePath('js/app.jsx'),
            ];
        }
    ...
    /**
     * Install GSManager Reverb into the application if desired.
     *
     * @return void
     */
    protected function installReverb()
    {
        if ($this->driver !== 'reverb' || $this->option('without-reverb') || InstalledVersions::isInstalled('gsmanager/reverb')) {
            return;
        }

        $install = confirm('Would you like to install GSManager Reverb?', default: true);
    ...
    protected function installNodeDependencies()
    {
        ...
        if (file_exists(base_path('pnpm-lock.yaml'))) {
            $commands = [
                'pnpm add --save-dev gsmanager-echo pusher-js',
                'pnpm run build',
            ];
        } elseif (file_exists(base_path('yarn.lock'))) {
            $commands = [
                'yarn add --dev gsmanager-echo pusher-js',
                'yarn run build',
            ];
        } elseif (file_exists(base_path('bun.lock')) || file_exists(base_path('bun.lockb'))) {
            $commands = [
                'bun add --dev gsmanager-echo pusher-js',
                'bun run build',
            ];
        } else {
            $commands = [
                'npm install --save-dev gsmanager-echo pusher-js',
                'npm run build',
            ];
        }
    ...
    {
        $packageJsonPath = $this->gsmanager->basePath('package.json');
