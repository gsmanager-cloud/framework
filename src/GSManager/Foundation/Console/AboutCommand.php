...
class AboutCommand extends Command
{
     ...
     *
     * @var \GSManager\Support\Composer
     */
    protected $composer;
     ...
     *
     * @param  \GSManager\Support\Composer  $composer
     */
        ...
        (new Collection(static::$data))
            ->map(fn ($items) => (new Collection($items))
                ->map(function ($value) {
                    ...
                    if (is_string($value)) {
                        $value = $this->gsmanager->make($value);
                    }

                    return (new Collection($this->gsmanager->call($value)))
     ...
     *
     * @param  \GSManager\Support\Collection  $data
     * @return void
     */
    ...
    protected function gatherApplicationInformation()
    {
        ...
        static::addToSection('Environment', fn () => [
            ...
            'GSManager Version' => $this->gsmanager->version(),
            'PHP Version' => phpversion(),
            'Composer Version' => $this->composer->getVersion() ?? '<fg=yellow;options=bold>-</>',
            'Environment' => $this->gsmanager->environment(),
            'Debug Mode' => static::format(config('app.debug'), console: $formatEnabledStatus),
            'URL' => Str::of(config('app.url'))->replace(['http://', 'https://'], ''),
            'Maintenance Mode' => static::format($this->gsmanager->isDownForMaintenance(), console: $formatEnabledStatus),
            'Timezone' => config('app.timezone'),
            'Locale' => config('app.locale'),
        ]);

        static::addToSection('Cache', fn () => [
            'Config' => static::format($this->gsmanager->configurationIsCached(), console: $formatCachedStatus),
            'Events' => static::format($this->gsmanager->eventsAreCached(), console: $formatCachedStatus),
            'Routes' => static::format($this->gsmanager->routesAreCached(), console: $formatCachedStatus),
