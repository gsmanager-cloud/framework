...
use GSManager\Foundation\Events\MaintenanceModeEnabled;
use GSManager\Foundation\Exceptions\RegisterErrorViewPaths;
...
class DownCommand extends Command
{
    ...
    {
        try {
            if ($this->gsmanager->maintenanceMode()->active() && ! $this->getSecret()) {
            ...
            $this->gsmanager->maintenanceMode()->activate($downFilePayload);
            ...
            $this->gsmanager->get('events')->dispatch(new MaintenanceModeEnabled());
    ...
    {
        try {
            return $this->gsmanager->make(PreventRequestsDuringMaintenance::class)->getExcludedPaths();
        } catch (Throwable) {
            return [];
        }
    }
