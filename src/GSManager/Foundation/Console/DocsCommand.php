...
use GSManager\Http\Client\Factory as Http;
...
use function GSManager\Prompts\suggest;
...
class DocsCommand extends Command
{
     ...
     * @var string
     */
    protected $description = 'Access the GSManager documentation';
     ...
     * @var string
     */
    protected $help = 'If you would like to perform a content search against the documentation, you may call: <fg=green>php gsm docs -- </><fg=green;options=bold;>search query here</>';
     ...
     *
     * @param  \GSManager\Http\Client\Factory  $http
    ...
    {
        if ($this->isSearching()) {
            return "https://gsmanager.ru/docs/{$this->version()}?".Arr::query([
                'q' => $this->searchQuery(),
            ]);
        }

        return with($this->page(), function ($page) {
            return trim("https://gsmanager.ru/docs/{$this->version()}/{$page}#{$this->section($page)}", '#/');
        });
    }
    ...
    protected function refreshDocs()
    {
        with($this->fetchDocs(), function ($response) {
            if ($response->successful()) {
                $this->cache->put("gsm.docs.{{$this->version()}}.index", $response->collect(), CarbonInterval::months(2));
            }
        });
    }

    /**
     * Fetch the documentation index from the GSManager website.
     *
     * @return \GSManager\Http\Client\Response
     */
    protected function fetchDocs()
    {
        return $this->http->get("https://gsmanager.ru/docs/{$this->version()}/index.json");
    }
    ...
    {
        return Str::before($this->version ?? $this->gsmanager->version(), '.').'.x';
    }
