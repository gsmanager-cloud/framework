...
class MySqlSchemaState extends SchemaState
{
    ...
    public function dump(Connection $connection, $path)
    {
        $this->executeDumpProcess($this->makeProcess(
            $this->baseDumpCommand().' --routines --result-file="${:GSMANAGER_LOAD_PATH}" --no-data'
    ...
    {
        $command = 'mysql '.$this->connectionString().' --database="${:GSMANAGER_LOAD_DATABASE}" < "${:GSMANAGER_LOAD_PATH}"';
    ...
    {
        $value = ' --user="${:GSMANAGER_LOAD_USER}" --password="${:GSMANAGER_LOAD_PASSWORD}"';

        $config = $this->connection->getConfig();

        $value .= $config['unix_socket'] ?? false
            ? ' --socket="${:GSMANAGER_LOAD_SOCKET}"'
            : ' --host="${:GSMANAGER_LOAD_HOST}" --port="${:GSMANAGER_LOAD_PORT}"';

        if (isset($config['options'][\PDO::MYSQL_ATTR_SSL_CA])) {
            $value .= ' --ssl-ca="${:GSMANAGER_LOAD_SSL_CA}"';
        }
    ...
    protected function baseVariables(array $config)
    {
        ...
        return [
            'GSMANAGER_LOAD_SOCKET' => $config['unix_socket'] ?? '',
            ...
            'GSMANAGER_LOAD_PASSWORD' => $config['password'] ?? '',
            'GSMANAGER_LOAD_DATABASE' => $config['database'],
            'GSMANAGER_LOAD_SSL_CA' => $config['options'][\PDO::MYSQL_ATTR_SSL_CA] ?? '',
        ];
    }
