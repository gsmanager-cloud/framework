...
use GSManager\Database\Events\DatabaseBusy;
...
class MonitorCommand extends DatabaseInspectionCommand
{
     ...
     *
     * @param  \GSManager\Database\ConnectionResolverInterface  $connection
    ...
    protected function parseDatabases($databases)
    {
        return (new Collection(explode(',', $databases)))->map(function ($database) {
            if (! $database) {
                $database = $this->gsmanager['config']['database.default'];
            }
     ...
     *
     * @param  \GSManager\Support\Collection  $databases
     * @return void
     */
