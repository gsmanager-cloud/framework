...
use GSManager\Database\Events\SchemaLoaded;
use GSManager\Database\Migrations\Migrator;
use GSManager\Database\SQLiteDatabaseDoesNotExistException;
use GSManager\Database\SqlServerConnection;
...
class MigrateCommand extends BaseCommand implements Isolatable
{
     ...
     * @var string
     */
    protected $description = 'Запустите миграцию базы данных';
    ...
    {
        if ($this->gsmanager['config']->get("database.connections.{$connection->getName()}.database") !== $connection->getDatabaseName()) {
            return false;
        }
        ...
        try {
            ...
            $this->gsmanager['config']->set("database.connections.{$connection->getName()}.database", $connection->getDatabaseName());
        }
    }
