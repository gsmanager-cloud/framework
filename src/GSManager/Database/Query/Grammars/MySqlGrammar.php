...
namespace GSManager\Database\Query\Grammars;

use GSManager\Database\Query\Builder;
use GSManager\Database\Query\JoinLateralClause;
...
class MySqlGrammar extends Grammar
{
     ...
     *
     * @param  \GSManager\Database\Query\Builder  $query
     * @param  \GSManager\Database\Query\IndexHint  $indexHint
     * @return string
     */
    ...
    protected function compileLegacyGroupLimit(Builder $query)
    {
        ...
        $partition = ', @gsmanager_row := if(@gsmanager_group = '.$column.', @gsmanager_row + 1, 1) as `gsmanager_row`';
        $partition .= ', @gsmanager_group := '.$column;
        ...
        $from = '(select @gsmanager_row := 0, @gsmanager_group := 0) as `gsmanager_vars`, ('.$sql.') as `gsmanager_table`';

        $sql = 'select `gsmanager_table`.*'.$partition.' from '.$from.' having `gsmanager_row` <= '.$limit;

        if (isset($offset)) {
            $sql .= ' and `gsmanager_row` > '.$offset;
        }

        return $sql.' order by `gsmanager_row`';
    }
    ...
    public function compileUpsert(Builder $query, array $values, array $uniqueBy, array $update)
    {
        ...
        $columns = (new Collection($update))->map(function ($value, $key) use ($useUpsertAlias) {
            ...
            return $useUpsertAlias
                ? $this->wrap($value).' = '.$this->wrap('gsmanager_upsert_alias').'.'.$this->wrap($value)
     ...
     *
     * @param  \GSManager\Database\Query\JoinLateralClause  $join
